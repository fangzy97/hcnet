...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='data/config/pascal/pascal_my_imp_14916_c_contrastive_v4_split0_resnet50.yaml', help='config file')
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if main_process():
            ...
            writer.add_scalar('TRAIN/aux_loss_train', aux_loss_train, epoch_log)
        ...
        if args.evaluate and (epoch % 2 == 0 or (args.epochs <= 50 and epoch % 1 == 0)):
            ...
            if main_process():
                ...
                writer.add_scalar('VAL/class_miou_val', class_miou, epoch_log)
            ...
            last_filename = args.save_path + '/train_epoch_' + str(epoch_log) + '.pth'
            torch.save({'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}, last_filename)
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        if args.base_lr > 1e-6:
            poly_learning_rate(optimizer, args.base_lr, current_iter, max_iter, power=args.power, index_split=index_split, warmup=args.warmup,
        ...
        subcls = subcls[0].cuda(non_blocking=True)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'Accuracy {accuracy:.4f}.'.format(epoch + 1, args.epochs, i + 1, len(train_loader),
                                                          ...
                                                          accuracy=accuracy))
        if main_process():
            writer.add_scalar('TRAIN-BATCH/loss_train', main_loss_meter.val, current_iter)
            writer.add_scalar('TRAIN-BATCH/aux_loss_train', aux_loss_meter.val, current_iter)
            writer.add_scalar('TRAIN-BATCH/mIoU_train', np.mean(intersection / (union + 1e-10)), current_iter)
            writer.add_scalar('TRAIN-BATCH/mAcc_train', np.mean(intersection / (target + 1e-10)), current_iter)
            writer.add_scalar('TRAIN-BATCH/allAcc_train', accuracy, current_iter)
