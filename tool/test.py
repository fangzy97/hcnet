...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='data/config/pascal/pascal_my_imp_14_c_split0_resnet50.yaml', help='config file')
...
def validate(val_loader, model, criterion, args):
    ...
    for e in range(20):
        for i, (input, target, s_input, s_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            output, amaps = model(s_x=s_input, s_y=s_mask, x=input, y=target)
            ...
            if args.save_fig:
                visualization(s_x=s_input, s_y=s_mask, x=input, y=target, pred=output, sub_cls=subcls[0].cpu().numpy()[0],
                              split_gap=split_gap, iter=i, amaps=amaps)
            # amap1 = F.interpolate(amap1.unsqueeze(1), size=target.size()[1:], mode='bilinear', align_corners=True)
            # amap2 = F.interpolate(amap2.unsqueeze(1), size=target.size()[1:], mode='bilinear', align_corners=True)
            # amap3 = F.interpolate(amap3.unsqueeze(1), size=target.size()[1:], mode='bilinear', align_corners=True)
...
def visualization(s_x, s_y, x, y, pred, sub_cls, split_gap, iter, amaps=None):
